...
- Branding: update launchd labels, mobile bundle IDs, and logging subsystems to bot.molt (legacy com.openclaw migrations). Thanks @thewilloftheshadow.
...
- Env: load global `$CLAWDBOT_STATE_DIR/.env` (`~/.openclaw/.env`) as a fallback after CWD `.env`.
